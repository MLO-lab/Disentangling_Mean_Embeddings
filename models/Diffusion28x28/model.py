...
class UNet(nn.Module):
    def __init__(self, nc=3, time_dim=256, image_size=64, deep_conv=False, size=8, device="cpu"):
        ...
        self.up1 = Up(size*64, size*16, uneven=image_size // 4 % 2)
        self.sa4 = SelfAttention(size*16, image_size // 4)
        self.up2 = Up(size*32, size*8, uneven=image_size // 2 % 2)
...
class Up(nn.Module):
    def __init__(self, in_channels, out_channels, emb_dim=256, uneven=False):
        super(Up, self).__init__()

        if uneven:
            self.up = nn.Upsample(scale_factor=2.33333333333333333333, mode="nearest", recompute_scale_factor=False)  # align_corners=True,
        else:
...
if __name__ == "__main__":
    import torch
    model = UNet(nc=1, image_size=28, deep_conv=False)
    n = torch.randn(5, 1, 28, 28)
    t = torch.tensor([5])
    # print(model)
    print(model(n, t=t).shape)
